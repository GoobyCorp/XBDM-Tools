...
class XBDMServerProtocol(asyncio.Protocol):
	...
	def connection_made(self, transport: asyncio.Transport):
		...
		self.transport.write(b"201- connected\r\n")
		...
		if raw_command:
			if raw_command.endswith(b"\r\n") and not self.receiving_files:
				...
				if parsed.name == "BOXID":
					...
					self.transport.write(b"420- box is not locked\r\n")
					...
					self.transport.write(b"200- " + bytes(cfg["console_name"], "utf8") + b"\r\n")
				elif parsed.name == "consoletype":
					print("Sending console type...")
					self.transport.write(b"200- " + bytes(cfg["console_type"], "utf8") + b"\r\n")
					...
					if parsed.param_exists("ver") and parsed.param_exists("type"): # is jrpc2 command
						...
						if type_param == 9:
							...
							self.transport.write(b"200- " + bytes(jrpc2cfg["KernelVers"], "utf8") + b"\r\n")
						elif type_param == 14:
							...
							if gettemp_params == "A\\0\\A\\1\\1\\0\\":
								...
								self.transport.write(b"200- " + bytes(hex(jrpc2cfg["CPUTemp"]).replace("0x",""), "utf8") + b"\r\n")
							elif gettemp_params == "A\\0\\A\\1\\1\\1\\":
								print("JRPC2 - Get GPU Temperature command received! Responding...")
								self.transport.write(b"200- " + bytes(hex(jrpc2cfg["GPUTemp"]).replace("0x",""), "utf8") + b"\r\n")
							elif gettemp_params == "A\\0\\A\\1\\1\\2\\":
								print("JRPC2 - Get EDRAM Temperature command received! Responding...")
								self.transport.write(b"200- " + bytes(hex(jrpc2cfg["EDRAMTemp"]).replace("0x",""), "utf8") + b"\r\n")
							elif gettemp_params == "A\\0\\A\\1\\1\\3\\":
								print("JRPC2 - Get MOBO Temperature command received! Responding...")
								self.transport.write(b"200- " + bytes(hex(jrpc2cfg["MOBOTemp"]).replace("0x",""), "utf8") + b"\r\n")
						elif type_param == 16:
							print("JRPC2 - Get TitleID command received! Responding...")
							self.transport.write(b"200- " + bytes(jrpc2cfg["TitleID"], "utf8") + b"\r\n")
						elif type_param == 17:
							print("JRPC2 - Get Mobo Type command received! Responding...")
							self.transport.write(b"200- " + bytes(jrpc2cfg["MoboType"], "utf8") + b"\r\n")
						elif type_param == 18:
					...
					self.transport.write(b"202- multiline response follows\r\n")
					lines = [
					...
					for single in lines:
						self.transport.write(bytes(single, "utf8") + b"\r\n")
					self.transport.write(b".\r\n")
				elif parsed.name == "XBEINFO" and parsed.flag_exists("RUNNING"):
					print("Sending current title info...")
					self.transport.write(b"202- multiline response follows\r\n")
					self.transport.write(b"timestamp=0x00000000 checksum=0x00000000\r\n")
					self.transport.write(b"name=\"" + bytes(cfg["current_title_path"], "utf8") + b"\"\r\n")
					self.transport.write(b".\r\n")
					...
					for single in list_drives():
						...
						self.transport.write(b"drivename=\"" + bytes(single, "utf8") + b"\"\r\n")
					self.transport.write(b".\r\n")
				elif parsed.name == "ISDEBUGGER":
					print("Requesting is debugger...")
					self.transport.write(b"410- name=\"XRPC\" user=" + bytes(cfg["username"], "utf8") + b"\r\n")
					...
					if parsed.flag_exists("set"):  # set kdnet settings
						if parsed.param_exists("IP") and parsed.param_exists("Port"):
							...
							self.transport.write(b'200- kdnet set succeeded.\r\n')
					elif parsed.flag_exists("show"):  # show settings
						self.transport.write(b'200- kdnet settings:\x1E\tEnable=1\x1E\tTarget IP: 192.168.0.43\x1E\tTarget MAC: 00-25-AE-E4-43-87\x1E\tHost IP: 192.168.0.2\x1E\tHost Port: 50001\x1E\tEncrypted: 0\x1E\r\n')
					...
					if parsed.param_exists("NAME"):
						...
						if isdir(physical_path):
							...
							for single in list_dirs(physical_path):
								...
								self.transport.write(b"name=\"" + single + b"\" sizehi=0x0 sizelo=0x0 createhi=0x01d3c0d2 createlo=0x40667d00 changehi=0x01d3c0d2 changelo=0x40667d00 directory\r\n")
							self.transport.write(b".\r\n")
						else:
							self.transport.write(b"402- directory not found\r\n")
					...
					if parsed.param_exists("NAME"):
						...
						if isfile(physical_path):
							...
							self.transport.write(b"402- file not found\r\n")
					...
					if parsed.param_exists("COUNT"):
						...
						if file_count > 0:
							self.transport.write(b"204- send binary data\r\n")
					...
					if parsed.param_exists("addr") and parsed.param_exists("data"):
						...
						self.transport.write(b"200- set " + bytes(str(len(setmem_data)), "UTF8") + b" bytes\r\n")
					...
					self.transport.write(b"205- now a notification channel\r\n")
					...
					self.transport.write(b"200- bye\r\n")
					self.transport.close()
				else:
